...
outfile=fopen('linear_elastic_brick8_results_allzero.txt','w');
...
function C = materialstiffness(ndof,ncoord,strain,materialprops)
  ...
  %if you put this in gigapascle value change the scale factor at postprocessing function to make the displacement visual
    C11 = 225e9; 
    C12 = 153e9; 
    C44 = 115e9;

    C = zeros(ndof,ncoord,ndof,ncoord); %stifness in new co-ordiante initialization
    C_ref = zeros(ndof,ncoord,ndof,ncoord); %stifness in reference co-ordiante initialization
   ...
   elseif (ncoord==3) % Plot a 3D mesh
       for lmn = 1:nelem
           ...
           elseif (nelnodes(lmn)==20)
            ...
            title('green undeformed plot, red is deformed')
       end    
   end
   axis equal
   hold off
